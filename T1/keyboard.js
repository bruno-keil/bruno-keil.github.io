...
function keyboardUpdate() {
  ...
  // Tank 2 movement
  if (keyboard.pressed("up")) tank2.translateZ(0.7);
  if (keyboard.pressed("down")) tank2.translateZ(-0.7);
  if (keyboard.pressed("left")) tank2.rotateY(angle);
  if (keyboard.pressed("right")) tank2.rotateY(-angle);
  if (keyboard.down("/")) shoot(tank2, scene, wallBoxes);
  if (keyboard.down(",")) shoot(tank2, scene, wallBoxes);
