...
let matrix = [
  ...
  function createArena2(size, scene) {
    ...
    for (let i = 0; i < matrix.length; i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        if (row[j] === 'w') {
          ...
          wallBoxes2.push(wallBox);
          walls2.push(cube);
        } else if (row[j] === 't1') {
          tank1.position.set(j * cubeSize - halfSize, 3, i * cubeSize - halfSize);
          scene.add(tank1);
        } else if (row[j] === 't2') {
          tank2.position.set(j * cubeSize - halfSize, 3, i * cubeSize - halfSize);
          scene.add(tank2);
        }
      }
    }
  ...
  function createFloor2(size, scene){
  ...
  export { createArena2, createFloor2, wallBoxes2, walls2 };